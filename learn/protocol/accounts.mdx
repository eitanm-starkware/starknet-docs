...
- [Legacy and deprecated](#legacy-and-deprecated)
  ...
  - [V1-bound accounts](#v1-bound-accounts)
...
Some Cairo 0 and Cairo 1 accounts were designed specifically around transaction version 1 and cannot easily migrate to version 3 transactions. To ensure backward compatibility for these accounts, the protocol supports **v1-bound accounts** — accounts that are permitted to continue using transaction version 1 even after the general deprecation of older transaction versions.

This mechanism allows existing accounts that depend on v1 transaction semantics (such as specific signature formats or fee payment structures) to remain functional without requiring contract upgrades or migrations.

**Do not rely on v1-bound account support for new account implementations.** New accounts should use transaction version 3 exclusively.
...
| v0.13.5 | V1-bound accounts introduced for backward compatibility with Cairo 0/1 accounts that depend on transaction version 1 semantics. |
