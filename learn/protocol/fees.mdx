...
Starting in Starknet 0.13.5, transactions may also specify resource bounds for _proving gas_, which enforces constraints related to the proving layer. Proving gas is a separate resource type that accounts for the computational costs of generating proofs for transaction execution. This allows the protocol to more accurately price transactions based on their proving complexity, separate from general L2 computation costs.
...
<Note>
  Starting in Starknet 0.13.5, v1-bound-accounts are introduced for Cairo 0 and Cairo 1 contracts to ensure backward compatibility with transaction version 1. These accounts use the legacy VM resource tracking mechanism.
</Note>
...
<Note>
  Starting in Starknet 0.13.5, builtin operation weights are used for more accurate gas computation. These weights reflect the actual computational cost of each builtin operation in the proving process.
</Note>
...
<Note>
  Starting in Starknet 0.13.5, VM resource tracking has been restructured to separate transaction resources from OS resources, providing more accurate fee calculations.
</Note>
...
<Note>
  Starting in Starknet 0.13.5, each transaction can emit up to 1,000 events. This limit is enforced by the versioned constants and affects how blocks are closed and transactions are priced.
</Note>
